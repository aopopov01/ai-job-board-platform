        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center min-h-screen py-20">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              {/* Search Results */}
              {showResults && (
                <div className="mt-6 p-6 bg-black/60 backdrop-blur-md border-2 border-white/20 rounded-2xl shadow-2xl">
...
function FeaturedJobsSection() {
  ...
  const handleApplyNow = (jobTitle: string, companyName: string) => {
    setApplicationForm({
      isOpen: true,
      jobTitle,
      companyName
            ...
            <div key={index} className="p-8 bg-black/60 backdrop-blur-md border-2 border-white/20 rounded-3xl shadow-2xl hover:border-blue-400/40 transition-all duration-300 group cursor-pointer">
              ...
              <div className="mt-6 pt-6 border-t border-white/20 space-y-3">
                <button 
                  ...
                  className="w-full h-12 bg-white/20 hover:bg-white/30 border-2 border-white/40 text-white rounded-xl font-bold transition-all duration-300"
                >
                  View Details
                </button>
                <button 
                  onClick={() => handleApplyNow(job.title, job.company)}
                  className="w-full h-12 bg-blue-600/20 hover:bg-blue-600 border-2 border-blue-400/40 hover:border-blue-400 text-blue-300 hover:text-white rounded-xl font-bold transition-all duration-300"
                >
                  Apply Now
                </button>
              </div>
