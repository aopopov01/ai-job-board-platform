  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      nodesRef.current.forEach((node, i) => {
        node.connections.forEach(connectionIndex => {
          ...
          if (node.type === 'core' || otherNode.type === 'core') {
            // Core connections - electric blue to cyan (increased contrast)
            gradient.addColorStop(0, `rgba(0, 150, 255, ${energy * pulse * 1.2})`)
            gradient.addColorStop(0.5, `rgba(100, 200, 255, ${energy * pulse * 1.4})`)
            gradient.addColorStop(1, `rgba(0, 255, 200, ${energy * pulse * 1.2})`)
            ctx.lineWidth = 2.0 + energy * 2.5
          } else if (node.type === 'spark' || otherNode.type === 'spark') {
            // Spark connections - golden electric (increased contrast)
            gradient.addColorStop(0, `rgba(255, 200, 0, ${energy * pulse * 0.9})`)
            gradient.addColorStop(0.5, `rgba(255, 150, 50, ${energy * pulse * 1.1})`)
            gradient.addColorStop(1, `rgba(255, 100, 100, ${energy * pulse * 0.9})`)
            ctx.lineWidth = 1.2 + energy * 2.0
          } else {
            // Synapse connections - cool blue (increased contrast)
            gradient.addColorStop(0, `rgba(50, 150, 255, ${energy * pulse * 0.8})`)
            gradient.addColorStop(1, `rgba(100, 200, 255, ${energy * pulse * 1.0})`)
            ctx.lineWidth = 1.4 + energy * 2.0
          }
      ...
      nodesRef.current.forEach(node => {
        ...
        // Node colors and effects (increased contrast)
        let nodeColor, coreColor, glowColor
        
        if (node.type === 'core') {
          nodeColor = `rgba(0, 200, 255, ${pulse * 1.4})`
          coreColor = `rgba(200, 240, 255, ${pulse * 1.3})`
          glowColor = 'rgba(0, 200, 255, 1.0)'
        } else if (node.type === 'spark') {
          nodeColor = `rgba(255, 180, 0, ${pulse * 1.3})`
          coreColor = `rgba(255, 220, 150, ${pulse * 1.2})`
          glowColor = 'rgba(255, 180, 0, 0.8)'
        } else {
          nodeColor = `rgba(100, 180, 255, ${pulse * 1.2})`
          coreColor = `rgba(180, 220, 255, ${pulse * 1.1})`
